...
from bd import engine, Session, write_msg, reg_user, add_user, add_user_photos, add_to_black_list, \
...
def menu_bot(id_num):
  write_msg(
    id_num, f"Вас приветствует бот - Vkinder\n"
    ...
    f"\nДля поиска - мужчина 30-35 Санкт-Петербург\n"
...
def show_info():
  write_msg(
    user_id, f'Это была последняя анкета.'
    f'Перейти в избранное - 2'
    f'Перейти в черный список - 0'
    f'Поиск - женщина 30-35 Воронеж'
    f'Меню бота - Vkinder')
        ...
        if user_photo == 'нет доступа к фото' or dating_user is not None or blocked_user is not None:
          ...
          try:
            ...
            if msg_text == '0':
              ...
              try:
                add_user(user_id, result[i][3], result[i][1], result[i][0],
                         city, result[i][2], current_user_id.id)
                #"""добавить фото анкеты в БД"""
